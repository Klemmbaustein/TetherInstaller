...
void ModsTab::GenerateModInfo()
{
	...
	std::string Markdown = Thunderstore::SelectedMod.Description;
	// Backslash.
	...
	for (auto& i : Lines)
	{
		...
		auto DescriptionText = new UIText(size, 1, SeperatorRemovedString, UI::Text);
		...
		MarkdownBackground->AddChild(DescriptionText);


		if (size >= 0.6f || i != SeperatorRemovedString)
		{
			MarkdownBackground->AddChild((new UIBackground(true, 0, 1, Vector2f(1.15, 0.005)))
